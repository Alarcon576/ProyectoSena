...
const URL_SOLICITUDES = "http://localhost:3000/api/solicitudes";
const URL_FAVORITOS   = "http://localhost:3000/api/favoritos";
...
function Adopciones({ onSwitch }) {
  ...
  const cargarMascotas = async () => {
    ...
    } catch (err) { console.error(err); setMascotas([]); }
  ...
  const toggleFavorito = async (id_mascota) => {
    ...
    } catch (err) { console.error("Error toggle favorito:", err); }
  ...
  const getBadge = (estado) => {
    ...
    return ESTADO_BADGE[key] || { label: estado || "Desconocido", cls: "badge--otro" };
  };
  ...
  const enviarSolicitud = async () => {
    ...
    try {
      ...
      setExito(true);
      setTimeout(() => { setModalSolicitud(null); setNotas(""); setExito(false); }, 2000);
    ...
    <>
          ...
          <div className="nav-avatar-wrapper" onClick={(e) => { e.stopPropagation(); setMenuAvatarAbierto((v) => !v); }}>
            ...
            {menuAvatarAbierto && (
              <div className="dropdown-avatar">
                ...
                <button onClick={() => onSwitch("configuracion")}>⚙️ Configuración</button>
                <div className="dropdown-avatar-divider" />
                <button className="dropdown-avatar-logout" onClick={() => { localStorage.removeItem("token"); onSwitch("login"); }}>🚪 Cerrar sesión</button>
              </div>
            )}
          </div>
        </div>
              ...
              <>
                ...
                <div className="solicitud-mascota-preview">
                  <div className="solicitud-avatar">
                    {modalSolicitud.fotos?.[0]?.url_foto ? <img src={modalSolicitud.fotos[0].url_foto} alt={modalSolicitud.nombre} /> : <span>🐾</span>}
                  </div>
                  <div>
                ...
                <textarea placeholder="Cuéntanos sobre tu hogar..." value={notas} onChange={(e) => setNotas(e.target.value)} rows={4} />
      ...
      <section className="adopt-hero">
        ...
        <div className="adopt-hero-content">
          <h1>Encuentra a tu alma<br />gemela peluda</h1>
          ...
          <div className="adopt-search-bar">
            ...
            <input type="text" placeholder="Buscar por nombre, raza o ubicación..." value={busqueda} onChange={(e) => setBusqueda(e.target.value)} />
      ...
      <div className="adopt-page">
        ...
        {loading ? (
          <div className="adopt-loading"><div className="adopt-spinner" /><p>Cargando mascotas...</p></div>
        ) : mascotasOrdenadas.length === 0 ? (
          <div className="adopt-empty">
            <span>🐾</span><p>No se encontraron mascotas con esos filtros</p>
            <button onClick={() => { setBusqueda(""); setFiltroEspecie("Todos"); setFiltroEdad("Todos"); setFiltroTamanio("Todos"); setFiltroGenero("Todos"); }}>Limpiar filtros</button>
          </div>
        ) : (
                ...
                <div className={`adopt-card ${!disponible ? "adopt-card--nodisponible" : ""}`} key={mascota.id_mascota}>
                  ...
                  <div className="adopt-card-body">
                    ...
                    <div className="adopt-card-actions">
                      <button className="btn-adoptar" disabled={!disponible} onClick={() => disponible && setModalSolicitud(mascota)}>Adoptar</button>
                      <button className="btn-info" onClick={() => setMascotaSeleccionada(mascota)}>Info</button>
                    </div>
                  </div>
                </div>
              );
      ...
      <footer className="adopt-footer">
        <div className="adopt-footer-inner">
          <div className="adopt-footer-brand">
            ...
            <div className="footer-social"><button>↗</button><button>♡</button><button>✉</button></div>
          </div>
          <div className="adopt-footer-links">
            <div><h4>Compañía</h4><span>Sobre Nosotros</span><span>Contacto</span><span>Nuestro Equipo</span></div>
            <div><h4>Ayuda</h4><span>Proceso de Adopción</span></div>
          </div>
        </div>
      </footer>
    </>
  ...
  return (
    <div className={`filter-dropdown ${isActive ? "filter-dropdown--active" : ""}`}>
      <button className="filter-dropdown-btn" onClick={(e) => { e.stopPropagation(); setOpen((v) => !v); }}>
        {isActive ? value : label} <span className="filter-dropdown-arrow">▾</span>
      </button>
      {open && (
        <div className="filter-dropdown-menu" onClick={(e) => e.stopPropagation()}>
          {options.map((opt) => (
            <div key={opt} className={`filter-dropdown-item ${value === opt ? "selected" : ""}`} onClick={() => { onChange(opt); setOpen(false); }}>{opt}</div>
          ))}
        </div>
      )}
    </div>
