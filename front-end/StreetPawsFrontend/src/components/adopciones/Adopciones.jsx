...
function Adopciones({ onSwitch, user }) {
  ...
  const getBadge = (estado) => {
    ...
    return (
      ESTADO_INFO[k] || { label: estado || "Desconocido", cls: "badge--otro" }
    );
  };
  ...
  const mascotasOrdenadas = esAdmin
    ...
    : mascotasFiltradas.filter(
        (m) => m.estado_adopcion?.toLowerCase().trim() === "disponible",
      );

  return (
    <>
      {/* ── NAVBAR UNIFICADA ── */}
      <Navbar onSwitch={onSwitch} activeView="adopciones" />

      {/* ── Modal proceso de adopción ── */}
      {modalProceso && (
        <div
          className="adopt-modal-overlay"
          onClick={() => setModalProceso(false)}
        >
          <div
            className="adopt-proceso-modal"
          ...
          >
            ...
            <div className="adopt-proceso-header">
              ...
              <p>
                Adoptar es fácil y seguro. Sigue estos pasos para darle un hogar
                a un animalito.
              </p>
            </div>
            <div className="adopt-proceso-pasos">
              {PASOS_ADOPCION.map((paso, i) => (
                <div className="adopt-proceso-paso" key={i}>
                  ...
                  {i < PASOS_ADOPCION.length - 1 && (
                    <div className="adopt-proceso-linea" />
                  )}
                </div>
              ))}
            </div>
            <button
              className="adopt-proceso-cta"
              onClick={() => setModalProceso(false)}
            >
        ...
        <>
          <section className="adopt-top-cta">
            <div className="adopt-top-cta-inner">
              ...
              <p>
                Agrega, edita y administra los animales en adopción, y revisa
        ...
        <>
          ...
          <div className="adopt-page">
            <div className="adopt-filters-wrap">
              <div className="adopt-filters-top">
                ...
                {hayFiltros && (
                  <button className="adopt-clear-btn" onClick={limpiarFiltros}>
                    ✕ Limpiar filtros
                  </button>
                )}
              </div>
          ...
          <footer className="adopt-footer">
            ...
            <div className="adopt-footer-bottom">
              © {new Date().getFullYear()} Street Paws · Hecho con 🐾 para los
              animales
            </div>
          </footer>
        </>
