  ...
  try {
    ...
    const usuario = await prisma.usuario.findUnique({
      ...
      select: {
        ...
        email: true,
        foto_perfil: true,
        descripcion: true   // ← incluida para perfil público
      }
    });
    if (!usuario) {
...
export const actualizarPerfil = async (req, res) => {
  try {
    ...
    // Solo actualizar descripcion si viene en el body
...
export const cambiarPassword = async (req, res) => {
  try {
    ...
    const hashNueva = await bcrypt.hash(contrasena_nueva, 10);

    await prisma.usuario.update({
      where: { id_usuario: req.user.id },
      data: { contrasena: hashNueva }
    });
